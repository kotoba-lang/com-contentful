(ns contentful.main
  "Kotodama WASM entrypoint for the Contentful clean-room actor (L5) — Clojure port.
  ...
  facts (`contentful.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "contentful")
(def tier "L5")
...
(def entity-specs
  [{:entity "Entry"        :plural "entries"       :id-prefix "contentf_ent"
    :fields [:type :version :space :environment :contentType :publishedVersion]
    :required [:type]
    :coerce {:version :int :publishedVersion :int}
    :refs {}}
   {:entity "Asset"        :plural "assets"        :id-prefix "contentf_ass"
    :fields [:type :version :space :environment :publishedVersion]
    :required [:type]
    :coerce {:version :int :publishedVersion :int}
    :refs {}}
   {:entity "ContentType"  :plural "contenttypes"  :id-prefix "contentf_con"
    :fields [:type :version :name :displayField :description]
    :required [:type]
    :coerce {:version :int}
    :refs {}}
   {:entity "Space"        :plural "spaces"        :id-prefix "contentf_spa"
    :fields [:type :version :name :organization]
    :required [:type]
    :coerce {:version :int}
    :refs {}}
   {:entity "Environment"  :plural "environments"  :id-prefix "contentf_env"
    :fields [:type :version :name :space :status]
    :required [:type]
    :coerce {:version :int}
    :refs {}}
   {:entity "Locale"       :plural "locales"       :id-prefix "contentf_loc"
    :fields [:type :version :name :code :fallbackCode :default :optional]
    :required [:type]
    :coerce {:version :int :default :bool :optional :bool}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"contentful.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "contentful-compat" :tier tier :entities entities} 200])
...
(defn start! [] :contentful-compat/ready)
